...
**Phaser Box2D passes checks 1, 2 and 4, and the physics side of check 3. It stays, pending one reading of the frame rate on a mid-range laptop.**
...
| 3 | Performance | 100 pebbles falling into a pile hold ≥ 60 fps on a mid-range laptop in Chrome | Physics step with 100 drawn pebbles: 1.0 ms mean, 7.4 ms p99, 8.5 ms max (a 60 fps frame has 16.7 ms). **Frame rate on a mid-range laptop: not yet measured** | Physics passes; fps pending |
...
To finish check 3: open the deployed build in Chrome on a mid-range laptop, press F1, click **Pebbles**, and read the fps while the pile forms. Add the reading and the laptop model above. If it holds 60 fps, this verdict is final. If not, file the Rapier swap as a follow-up issue.
