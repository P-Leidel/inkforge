...
The issues form a chain, in the order of the table: each is blocked by the one before it. The [architecture review after #16](../adr/reports/architecture-review-2026-09-25.html) inserted two refactors before #17, both done: #24 (Arena contents in their own modules, its candidate 1) and #27 (the Contact ledger, its candidate 2). #17 (Green), #18 (Spills and Patches) and #19 (Red Objects and Blasts) are done too, so #20 comes next. Start an issue only once the one before it is closed and CI is green on `main`. To start one, open a session on this repository and say:
