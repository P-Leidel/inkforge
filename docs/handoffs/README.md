...
## Patterns the slices follow (as of #17)

- **Arena contents are kinds.** Each kind of Arena contents (`CONTEXT.md`) is a module of its own behind the shared `Kind` shape in `src/sandbox/arena-contents.ts`. Today there are three:
...
- **One list of kinds.** The Sandbox world holds the kinds in one fixed list, in rebuild order: `Kinds` and `this.kinds` in `sandbox-world.ts`, today Strokes, Rubble, then Bonds. A new kind is one new module added to that list after its hosts. The snapshot, rebuilding, Clear, the Stroke context, the table re-apply, host gone and the step then cover it with no other change, and registering its Parties covers its contacts. `world.contents` holds every kind's views by kind name (`contents.strokes.lines`, `contents.rubble`, `contents.bonds`); it is only Arena contents, and the renderer doesn't use it.
- **Host gone.** Whatever is attached to a body goes when the body goes. Every removal goes through the ledger (`unregister`), which logs the Party; the world drains the log (`contacts.takeGone()`) in `passOnGone` and hands the Parties to every kind's `gone(parties)`, in kind order: at the end of each step, before the kinds' turns, and after `remove` and `undo`. Clear drops the log, since every kind clears itself. `Bonds.gone` removes each bond with a side gone, and the green Object falls free. A rebuild forgets Parties without them going.
...
- **Step order.** `SandboxWorld.step()`:
  ...
  4. breaks what the damage rules returned, with `breakTarget`;
  5. applies glue drag (`src/sandbox/glue.ts`) to what touches glue, `glue.apply(strokes.pieces(), dt)`, and breaks the Pieces it wore out with `breakTarget`;
  6. passes on what the step removed (`passOnGone`, host gone above), and then gives each kind its turn, in list order. (Rubble's turn ages the fading ghosts.)
...
- **The Contact ledger** (`ContactLedger` in `src/sandbox/contact-ledger.ts`, the world's `contacts`) decides which contacts count, for every rule. It is fed each step's report and never reads the engine; it asks physics only whether a sliding Object still slides. After each step it gives three channels, in the engine's report order, in arrays reused from step to step:
  ...
  It also looks a registered Party up by its id (`party(id)`), and logs every Party unregistered (`takeGone()`, host gone above).
