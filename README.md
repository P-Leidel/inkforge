...
While the Demolition demo runs, F1 also shows the average fps and the longest frame since it started (from the frame after it was loaded, and again from the start after R, counting only frames while physics runs), so milestone 2's exit criterion, at least 60 fps on average and no frame over 33 ms, can be read in the browser. `npm run verdict` times the same scene headless: the whole Sandbox world step over the run and during the chain, and one read of every view per step, as each frame's drawing makes.
