 ...
 * for the engine verdict (ADR 0001). Physics is deterministic, so the
 * tunnelling, stacking and stability results hold on any machine; step
 * times depend on this machine, and the frame rate must still be read from
 * the F1 overlay in Chrome on a mid-range laptop.
 *
 *   npm run verdict
 */
